...
const filterDiv = document.getElementById("filterDiv") as HTMLDivElement;

// PRICE FILTERS
const lowPrice = document.getElementById("lowPriceBound") as HTMLInputElement;
const highPrice = document.getElementById("highPriceBound") as HTMLInputElement;

// REVIEW FILTERS
const lowReview = document.getElementById("lowReviewBound") as HTMLInputElement;
const highReview = document.getElementById("highReviewBound") as HTMLInputElement;

// FILTER BUTTONS
const priceButton = document.getElementById("filterPrice") as HTMLButtonElement;
const reviewButton = document.getElementById("filterReview") as HTMLButtonElement;

// FILTERS
const chinese = document.getElementById('Chinese') as HTMLButtonElement;
const burger = document.getElementById('Burger') as HTMLButtonElement;
const mexican = document.getElementById('Mexican') as HTMLButtonElement;
const korean = document.getElementById('Korean') as HTMLButtonElement;
const japanese = document.getElementById('Japanese') as HTMLButtonElement;
const coffee = document.getElementById('Coffee') as HTMLButtonElement;
const italian = document.getElementById('Italian') as HTMLButtonElement;
const indian = document.getElementById('Indian') as HTMLButtonElement;
const pizza = document.getElementById('Pizza') as HTMLButtonElement;

// array that allows iteration through all filter buttons
let filtersArray: HTMLButtonElement[] = [chinese, burger, mexican, korean, japanese, coffee, italian, indian, pizza];

// INPUTS FOR CATEGORY SORTING
const restaurantSort = document.getElementById("Restaurant") as HTMLInputElement;
const costSort = document.getElementById("Cost") as HTMLInputElement;
const xSort = document.getElementById("X-Coordinate") as HTMLInputElement;
const ySort = document.getElementById("Y-Coordinate") as HTMLInputElement;
const reviewSort = document.getElementById('Review') as HTMLInputElement; 

const mapDisplay = document.getElementById("map-display") as HTMLDivElement;
const frugalMapDisplay = document.getElementById("frugal-display") as HTMLDivElement;

const timeDisplay = document.getElementById("restaurant-list-time-display") as HTMLParagraphElement;
...
let restaurantDisplayStart: number = 0;
...
function loadRestaurants(arr: number[]): void{
  for (let i = 0; i < 5; i++) {
    let index = i + restaurantDisplayStart;
    if (data.storeName[arr[index]] !== undefined) {
      // for the header
      let restaurantHeader = document.getElementById("restaurant-header-" + (i+1)) as HTMLHeadingElement;
      restaurantHeader.innerHTML = data.storeName[arr[index]];
  
      // displaying rating
      let restaurantRating = document.getElementById("restaurant-rating-" + (i+1)) as HTMLDivElement;
      ...
      let restaurantPrice = document.getElementById("restaurant-price-" + (i+1)) as HTMLHeadingElement;
      ...
      // displaying cuisine
      let restaurantCuisine = document.getElementById("restaurant-cuisine-" + (i+1)) as HTMLParagraphElement;
      restaurantCuisine.innerHTML = data.type[arr[index]];
    } 
  }
}

function drawRestaurants(arr: number[]): void { // O(1)
  for (let i = 0; i < 5; i++) {
    const index: number = i + restaurantDisplayStart;
    ...
    const newLocationPing = document.getElementById("restaurant-location-information-" + i) as HTMLDivElement;

    newLocationPing.style.left = "" + posX;
    newLocationPing.style.top = "" + posY;

    const newLocationPingName = document.getElementById("restaurant-name-" + i) as HTMLParagraphElement;
    newLocationPingName.innerText = data.storeName[arr[index]];

    const newLocationPingInfo = document.getElementById("restaurant-info-" + i) as HTMLParagraphElement;
...
function next(): void { // O(1)  
  // will not allow exceeding the list of restaurants
  if (restaurantDisplayStart >= displayValues.length) return;
...
function last(): void { // O(1)
  // will not allow Stack underflow
  if (restaurantDisplayStart === 0) return;
  restaurantDisplayStart -= 5;
...
function displayTime(time: number, time2: number): void {
...
// TOTAL (O(n log n))
function merge(arr1: any[], arr2: any[], arr3: any[], arr4: any[], arr5: any[], arr6: any[], arr7: any[],
  left: number, mid: number, right: number): [any[], any[], any[], any[], any[], any[], any[]]  {
  ...
  const sortArr1: any[] = new Array(arraySize);
  const sortArr2: any[] = new Array(arraySize);
  const sortArr3: any[] = new Array(arraySize);
  const sortArr4: any[] = new Array(arraySize);
  const sortArr5: any[] = new Array(arraySize);
  const sortArr6: any[] = new Array(arraySize);
  const sortArr7: any[] = new Array(arraySize);

  let i = left;
  let j = mid+1;

  for (let k = left; k <= right; k++) {
    let index: number = k-left;
    if (i > mid) {
      //If left is empty
...
function sortAscending(arr1: any[], arr2: any[], arr3: any[], arr4: any[], arr5: any[], arr6: any[], arr7: any[],
  l: number=0, r: number=arr1.length-1): [any[], any[], any[], any[], any[], any[], any[]] {
  if (l===r) return [arr1, arr2, arr3, arr4, arr5, arr6, arr7]; // array length is 1
...
function mergeDescending(arr1: any[], arr2: any[], arr3: any[], arr4: any[], arr5: any[], arr6: any[], arr7: any[],
  left: number, mid: number, right: number): [any[], any[], any[], any[], any[], any[], any[]]  {
...
/* l is for left index and r is right index 
of the sub-array of arr to be sorted */
function sortDescending(arr1: any[], arr2: any[], arr3: any[], arr4: any[], arr5: any[], arr6: any[], arr7: any[],
  l: number=0, r: number=arr1.length-1): [any[], any[], any[], any[], any[], any[], any[]] { // O(log n)
  if (l===r) return [arr1, arr2, arr3, arr4, arr5, arr6, arr7]; // array length is 1
...
// SEARCH FUNCTION
function binarySearch(arr: any[], target: any): number { // (O log n)
  ...
  while (l <= r) {
    ...
    else if (arr[mid] < target) l = mid+1; // target is before arr[mid]
    else r = mid-1; // if target is after arr[mid]
  }
  // if nothing is found
  return -1;
}
...
let searchCategory: string = "Restaurant";
function setSearchCategory(category: string): void { // O(1)
  // fetch button name to be changed on search category change
  const buttonName = document.getElementById("dropdown-button") as HTMLButtonElement;
  // fetch search value
...
function search(): number {
  ...
  let sortTimeStamp0: number = 0;
  let sortTimeStamp1: number = 0;

  let timeStamps: number[] = [0, 0]

  let searchTimeStamp0: number = 0;
  let searchTimeStamp1: number = 0;
  

  let result: number = -1; // index where search value will be stored
...
function onSort(buttonID: string | null): void { // O(1)
  // removing filters
  removeFilters();

  // array of buttons
  let sortButtonIDs: string[] = ["ascendingRestaurant", "ascendingCost", "ascendingReview", "ascendingX", "ascendingY",
  ...
  for (let i = 0; i < sortButtonIDs.length; i++) {
    ...
    const newID: string = sortButtonIDs[i];
    const newButton = document.getElementById(newID) as HTMLButtonElement;
  ...
  if (buttonID !== null) {
    const clickedButton: HTMLButtonElement = document.getElementById(buttonID) as HTMLButtonElement;
    clickedButton.style.backgroundColor = "red";
  }
}

// SORTING FUNCTIONS --> all O(1) time complexity (excluding the sorting algorithm function calls)
function ascendingID(): number[] {
...
function ascendingRestaurant(): number[] {
  let t0: number = performance.now();
...
function descendingRestaurant(): void{
  let t0 = performance.now();
  let array: any[][] = sortDescending(data.storeName, data.ID, data.type, data.x, data.cost, data.review, data.y);
...
function descendingCost(): void {
  let t0 = performance.now();
  let array: any[][] = sortDescending(data.cost, data.ID, data.type, data.x, data.y, data.review, data.storeName);
  let t1 = performance.now();
...
function descendingX(): void {
  let t0 = performance.now();
  let array: any[][] = sortDescending(data.x, data.ID, data.type, data.y, data.cost, data.review, data.storeName);
  let t1 = performance.now();
...
function descendingReview(): void {
  // Start timing the performance
  let t0 = performance.now();
...
function addFilter(filter: string, button: HTMLButtonElement): void { // O(n)
  ...
  if (cuisines.length > 0) displayValues = cuisines;
  else alert("No results match your filtered results!");
...
function removeFilters(): void {  // O(1)
...
function priceFilter(): void { // O(n)
  let priceFilterIndexes: number[] = [];
...
function reviewFilter(): void {
  reviewButton.disabled = true;
  reviewButton.style.backgroundColor = "red";
  let reviewFilterIndexes: number[] = [];
...
function singleArrayMerge(arr: number[], left: number, mid: number, right: number): number[] {
  let sorted: number[] = new Array(right - left + 1);
...
function singleArrayMergeSort(arr: number[], l: number=0, r: number = arr.length-1): number[] {
  if (arr.length === 0) return arr; // handling bad inputs
  if (l === r) return arr; // base case for recursion

  let mid: number = Math.floor((r + l) / 2);
...
function intersections(arr1: number[], arr2: number[], button: HTMLButtonElement): void {
...
let frugalCoords: number[][] = [];

frugalMapDisplay.addEventListener("click", (e) => {
  if (userPing < 2) {
    // drawing the point where the user clicks
    const newPing = document.getElementById("travel-ping-" + userPing) as HTMLImageElement;
    ...
    if (userPing === 1) {
      let allPoints: number[][] = createPoints(data.x, data.y);
      let path: number[][] = findRestaurantsOnPath(allPoints, frugalCoords[0], frugalCoords[1]);
      
      for (let i = 0; i < 4; i++) {
        const posX = path[i][0];
        const posY = path[i][1];

        const indexOfPoint: number = path[i + 4][0];

        const newRestaurant = document.getElementById("frugal-location-information-"+i) as HTMLDivElement;
        ...
        const newLocationPingName = document.getElementById("frugal-name-" + i) as HTMLParagraphElement;
        newLocationPingName.innerText = data.storeName[indexOfPoint];

        const newLocationPingInfo = document.getElementById("frugal-info-" + i) as HTMLParagraphElement;
        newLocationPingInfo.innerText = data.type[indexOfPoint] + 
                                    "\n$" + data.cost[indexOfPoint] 
                                    + "\n" + data.x[indexOfPoint] + ", " + data.y[indexOfPoint];
        
        const newRestaurantPing = document.getElementById("frugal-ping-" + i) as HTMLImageElement;
...
function removeMapPoints(): void { // O(1)
  // resets the ping counts
  userPing = 0;

  // iterates through the two pings, hides them
  for (let i = 0; i < 2; i++) {
    const newPing = document.getElementById("travel-ping-" + i) as HTMLImageElement;
    newPing.hidden = true;
  }
...
function createPoints(xValues: number[], yValues: number[]): number[][] { // O(n)
...
function distance(points1: number[], points2: number[]): number { // O(1)
...
function findRestaurantsOnPath(points: number[][], start: number[], end: number[]): number[][] { // O(n)
  // to store the path
  let path: number[][] = [];

  // finding total distance between start and end points
  const distanceOfPoints: number = distance(start, end); 
